...
const Dashboard = () => {
    ...
    const [editForm, setEditForm] = useState({
        id: '', title: '', deadline: '', description: '', priority: '', color: '', attachment: null
    });
    ...
    const [form, setForm] = useState({
        subject: '', title: '', description: '', deadline: '', priority: 'Biasa', color: '#3b82f6', attachment: null
    });
    ...
    const handleSave = async (e) => {
        ...
        try { 
            await axios.post('http://localhost:5001/tasks', data, {
                headers: { 
                    Authorization: `Bearer ${token}`
                }
            });
            setShowAddModal(false);
            setForm({ subject: '', title: '', description: '', deadline: '', priority: 'Biasa', color: '#3b82f6', attachment: null });
    ...
    const handleUpdate = async (e) => {
        ...
        data.append('deadline', editForm.deadline);
        data.append('priority', editForm.priority);
        data.append('color', editForm.color);
        if(editForm.attachment instanceof File) {
            data.append('attachment', editForm.attachment);
        }

        try { 
            await axios.patch(`http://localhost:5001/tasks/${editForm.id}`, data, {
                headers: { 
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const toggleComplete = async (task) => {
        const token = await refreshToken();
        const data = new FormData();
        // Sertakan data yang ada agar tidak terhapus di backend
        data.append('title', task.title);
        data.append('description', task.description || '');
        data.append('deadline', task.deadline);
        data.append('priority', task.priority);
        data.append('color', task.color);
        data.append('isCompleted', !task.isCompleted);

        try { 
            await axios.patch(`http://localhost:5001/tasks/${task.id}`, data, {
                headers: { 
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleUpdateProfile = async (e) => {
        ...
        const token = await refreshToken();
        try {
            await axios.patch(`http://localhost:5001/users/${user.id}`, profileForm, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleFileAction = (filename) => {
        ...
        if(imageExtensions.includes(ext)) {
            ...
            window.open(url, '_self'); // Mengarahkan ke file untuk memicu download/view PDF bawaan browser
        }
    };
                        ...
                        {!isEditing ? (
                            ...
                            <form onSubmit={handleUpdate}>
                                ...
                                <div className="input-group">
                                    ...
                                    <input type="file" onChange={e => setEditForm({...editForm, attachment: e.target.files[0]})} />
                                </div>
                                <div className="detail-actions">
